...
export const requiredValidator = (value: unknown) => {
  ...
  return !!String(value).trim().length || 'Campo obbligatorio'
}
...
export const fileValidator = (value: FileList) => {
  ...
  return value[0].type === 'application/pdf' || 'Il file deve essere un PDF'
}
...
export const emailValidator = (value: unknown) => {
  ...
  if (Array.isArray(value))
    return value.every(val => re.test(String(val))) || 'La mail deve essere valida'

  return re.test(String(value)) || 'La mail deve essere valida'
}
